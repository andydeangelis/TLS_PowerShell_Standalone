...
if ($RestoreToDefaults) {
    ...
    if ($regFolders) { 
        ...
        $validate1 = $(Write-Host "Are you sure you want to remove the current settings (Y/N)? " -ForegroundColor Red -NoNewline; Read-Host)
        if (($validate1 -ne 'Y') -or ($validate1 -ne 'y')) { 
            Write-Host "Invalid entry! Exiting script without applying changes." -ForegroundColor Red
            exit
        }

        $validate2 = $(Write-Host "Are you REALLY, REALLY sure you want to remove the current settings (Y/N)? " -ForegroundColor Red -NoNewline; Read-Host)
...
"GraceSeconds"=dword:00000000'
        ...
        Remove-Item .\sccmReboot.reg -Force
    }
...
elseif ((Get-ItemProperty -Path 'HKLM:\System\CityNationalBank\TLSControls' -ErrorAction SilentlyContinue).TLSControlsApplied -ne 1) {
    ...
    if (-not (Test-Path "$regBackupPath$dateTime" -ea SilentlyContinue)) { New-Item -Path "$regBackupPath\$dateTime" -Force -ItemType Directory }

    Invoke-Command { reg export "hkey_local_machine\SOFTWARE\Microsoft\.NETFramework\v4.0.30319" "C:\scripts\TLS\backup\$dateTime\dotNet4_x64_pre_tls12_backup_$($dateTime).reg" /y }
    Invoke-Command { reg export "HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\.NETFramework\v4.0.30319" "C:\scripts\TLS\backup\$dateTime\dotNet4_x86_pre_tls12_backup_$($dateTime).reg" /y }
    Invoke-Command { reg export "hkey_local_machine\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL" "C:\scripts\TLS\backup\$dateTime\schannel_pre_tls12_backup_$($dateTime).reg" /y }
